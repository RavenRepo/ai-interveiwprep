...
class InterviewServiceTest {
    ...
    class StartInterviewTests {
        ...
        void testStartInterview_Success() {
            ...
            // Act
            InterviewDTO result = interviewService.startInterview(1L, 10L, 5L);
            ...
            verify(interviewRepository).save(argThat(interview ->
                    interview.getStatus() == InterviewStatus.GENERATING_VIDEOS));

            verify(openAIService).generateInterviewQuestions("Experienced Java developer", "Software Engineer");
            verify(questionRepository, times(2)).save(any(Question.class));

            // P1: Verify QuestionsCreatedEvent was published (not AvatarVideoService called)
        ...
        void testStartInterview_UserNotFound() {
            ...
            assertThatThrownBy(() -> interviewService.startInterview(999L, 10L, 5L))
        ...
        void testStartInterview_ResumeMismatch() {
            ...
            otherUser.setId(2L);
            Resume otherResume = new Resume(11L, otherUser, "other.pdf", "resumes/2/other.pdf", "text", LocalDateTime.now());

            when(userRepository.findById(1L)).thenReturn(Optional.of(testUser));
            when(resumeRepository.findById(11L)).thenReturn(Optional.of(otherResume));

            assertThatThrownBy(() -> interviewService.startInterview(1L, 11L, 5L))
                    .isInstanceOf(RuntimeException.class)
        ...
        void testStartInterview_EmptyResumeText() {
            Resume emptyResume = new Resume(10L, testUser, "resume.pdf", "resumes/1/resume.pdf", null, LocalDateTime.now());

            when(userRepository.findById(1L)).thenReturn(Optional.of(testUser));
            when(resumeRepository.findById(10L)).thenReturn(Optional.of(emptyResume));

            assertThatThrownBy(() -> interviewService.startInterview(1L, 10L, 5L))
                    .isInstanceOf(RuntimeException.class)
        ...
        void testStartInterview_NoDirectAvatarCall() {
            ...
            // Act
            interviewService.startInterview(1L, 10L, 5L);
    ...
    class GenerateUploadUrlTests {
        ...
        void testGenerateUploadUrl_Success() {
            ...
            verify(videoStorageService).generatePresignedPutUrl("interviews/1/100/response_200_ts.webm", "video/webm");
        }
    ...
    class SubmitResponseTests {
        ...
        void testSubmitResponse_Success() {
            ...
            verify(responseRepository).save(argThat(r ->
                    "interviews/1/100/response_200_ts.webm".equals(r.getVideoUrl())));
            // P1: transcription is triggered with S3 key (SpeechToTextService resolves internally)
            verify(speechToTextService).transcribeVideoAsync("interviews/1/100/response_200_ts.webm", 200L);
        }
    ...
    class CompleteInterviewTests {
        ...
        void testCompleteInterview_Success() {
            ...
            verify(interviewRepository).save(argThat(interview -> interview.getStatus() == InterviewStatus.PROCESSING
